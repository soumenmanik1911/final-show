...
export const DashboardUserButton = () => {
  ...
  const handleLogout = async () => {
    setIsLoggingOut(true);
    // ✅ CHANGED: using Better Auth's own onSuccess/onError callbacks
    // instead of blindly waiting 2 seconds and hoping it worked
    await authClient.signOut({
      fetchOptions: {
        onSuccess: () => {
          router.push('/sign-in'); // ✅ only redirects when session is actually destroyed
        },
        onError: (ctx) => {
          console.error('Logout failed:', ctx.error);
          setIsLoggingOut(false); // ✅ resets state if it fails
        },
      },
    });
  };
