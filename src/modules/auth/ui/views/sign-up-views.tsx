...
import * as React from 'react';
import { useRouter } from 'next/navigation'; // ✅ ADDED: for redirect after signup
import { Card, CardContent, CardHeader, CardTitle, CardDescription, CardFooter } from '@/components/ui/card';
import { Input } from '@/components/ui/input';
import { authClient } from '@/lib/auth-client';
...
export const SignUpView = () => {
  ...
  const [isSubmitting, setIsSubmitting] = React.useState(false);
  const [error, setError] = React.useState<string | null>(null);
  // ✅ REMOVED: successMessage state — no longer needed since we redirect
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      // ✅ CHANGED: now we check the result for silent errors (Better Auth pattern)
      ...
      // ✅ ADDED: handle silent failure from Better Auth
      if (result?.error) {
        setError(result.error.message || 'Unable to sign up. Please try again.');
        return;
      }

      // ✅ CHANGED: redirect to home instead of showing success message
      // Session is already set at this point, user will be logged in
      router.push('/');
    ...
    <div className="min-h-screen flex items-center justify-center bg-white">
      <Card className="w-full max-w-3xl flex flex-col md:flex-row rounded-xl overflow-hidden shadow-2xl border-none bg-zinc-900">
        ...
        <CardContent className="flex flex-col justify-center md:basis-1/2 px-8 py-10">
          ...
          {/* ✅ REMOVED: success message block — replaced by redirect */}
